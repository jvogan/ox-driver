...
async function fixture({ docsFailed = false } = {}) {
	...
	const workers = [
		...
		{ ...compactWorkerReceipt(completedRun, completedWorkspace.path, "docs"), expectedHarness: "opencode", laneId: "docs", worktreeId: completedWorkspace.id, baseCommit: completedWorkspace.baseCommit },
	];
	const effectivePlan = validateEffectiveRetryPlan({ version: 1, lanes: [
		...
		{
			id: "docs", role: "docs", objective: "Document the result", workerPath: completedWorkspace.path,
			...
			worktreeId: completedWorkspace.id, baseCommit: completedWorkspace.baseCommit,
		},
	] });
...
process.stdout.write(JSON.stringify({
  ...
  changedPaths: ["result.txt"], unownedChangedPaths: [], finalOutput: "repair completed",
}));
...
test("retry-failed alias excludes completed siblings and rejects legacy receipts before spawn", async () => {
	const value = await fixture();
	const result = await execFileAsync(process.execPath, [
	...
	const legacy = await store.persist({
		...
		objective: "legacy", status: "failed", workers: value.sourceReceipt.workers, autoMerged: false,
	});
	const sentinel = join(value.root, "sentinel");
	await assert.rejects(execFileAsync(process.execPath, [
		"scripts/ox_orchestration.mjs", "retry", legacy.orchestrationId, "--failed",
...
test("effective plans retain their pinned provided-key hash", () => {
	const laneInput = {
		id: "lane-1", role: "builder", objective: "fixture objective", workerPath: "/workers/lane-1",
		route: "opencode-default", ownedPaths: ["."], excludedPaths: [".env", ".git"], checks: ["node --test"],
		timeoutSeconds: 600, reportOnlyCostUsdMicros: 10_000,
	};
	...
	assert.throws(
		() => validateEffectiveRetryPlan({ version: 1, lanes: [{ ...laneInput, harness: "unsupported" }] }),
