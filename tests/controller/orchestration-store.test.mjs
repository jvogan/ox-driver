...
test("accepts retry receipts written before per-worker back-references existed", async () => {
	const root = await trackedMkdtemp(join(tmpdir(), "ox-orchestration-earlier-retry-"));
	const workerRoot = await trackedMkdtemp(join(tmpdir(), "ox-orchestration-earlier-retry-worker-"));
	const store = new OrchestrationReceiptStore(root);
	const allocation = await store.allocate();
	...
	const earlier = await store.persist({
		version: 1,
		kind: "retry",
		orchestrationId: allocation.orchestrationId,
		receiptPath: allocation.receiptPath,
...
test("accepts herd receipts written before expectedHarness existed", async () => {
	const root = await trackedMkdtemp(join(tmpdir(), "ox-orchestration-earlier-herd-"));
	const workerRoot = await trackedMkdtemp(join(tmpdir(), "ox-orchestration-earlier-herd-worker-"));
	const store = new OrchestrationReceiptStore(root);
	const allocation = await store.allocate();
	// The compatibility release recorded harness on the plan lane only for Pi lanes and set it on
	// the worker summary the same way; expectedHarness did not exist yet.
	const effectivePlan = validateEffectiveRetryPlan({ version: 1, lanes: [
		{
			id: "docs", role: "writer", objective: "Write the docs pass", workerPath: join(workerRoot, "docs"),
			route: "opencode-default", ownedPaths: ["docs/"], excludedPaths: [".env"],
			checks: [], timeoutSeconds: 3600, reportOnlyCostUsdMicros: 50_000,
		},
		{
			id: "audit", role: "reviewer", objective: "Review the docs pass", workerPath: join(workerRoot, "audit"),
			harness: "pi", route: "pi-protected-inherited", ownedPaths: ["review.md"], excludedPaths: [".env"],
			checks: [], timeoutSeconds: 3600, reportOnlyCostUsdMicros: 50_000,
		},
	] });
	const earlier = await store.persist({
		...
		orchestrationId: allocation.orchestrationId,
		receiptPath: allocation.receiptPath,
		objective: "Mixed herd",
		status: "completed",
		effectivePlan,
		effectivePlanSha256: effectiveRetryPlanSha256(effectivePlan),
		workers: [
			{
				// The compatibility release left OpenCode harness undefined, which JSON drops.
				workerPath: join(workerRoot, "docs"),
				role: "writer",
				laneId: "docs",
				runId: randomUUID(),
				status: "completed",
			...
			{
				// Pi worker: harness present, expectedHarness absent.
				workerPath: join(workerRoot, "audit"),
				role: "reviewer",
				laneId: "audit",
				harness: "pi",
				runId: randomUUID(),
	...
	assert.equal(earlier.kind, "herd");
	assert.equal((await store.inspect(allocation.orchestrationId)).orchestrationId, allocation.orchestrationId);
	const listing = await store.list();
	assert.deepEqual(listing.unreadable, []);
});

test("still enforces effective-lane harness identity for receipts that declare expectedHarness", async () => {
	const root = await trackedMkdtemp(join(tmpdir(), "ox-orchestration-harness-"));
	const workerRoot = await trackedMkdtemp(join(tmpdir(), "ox-orchestration-harness-worker-"));
	const store = new OrchestrationReceiptStore(root);
	const effectivePlan = validateEffectiveRetryPlan({ version: 1, lanes: [
		{
			id: "docs", role: "writer", objective: "Write the docs pass", workerPath: join(workerRoot, "docs"),
			harness: "pi", route: "pi-protected-inherited", ownedPaths: ["docs/"], excludedPaths: [".env"],
			checks: [], timeoutSeconds: 3600, reportOnlyCostUsdMicros: 50_000,
		},
		{
			id: "audit", role: "reviewer", objective: "Review the docs pass", workerPath: join(workerRoot, "audit"),
			route: "opencode-default", ownedPaths: ["review.md"], excludedPaths: [".env"],
			checks: [], timeoutSeconds: 3600, reportOnlyCostUsdMicros: 50_000,
		},
	] });
	const planSha256 = effectiveRetryPlanSha256(effectivePlan);
	const worker = (laneId, overrides = {}) => ({
		workerPath: join(workerRoot, laneId),
		role: laneId === "docs" ? "writer" : "reviewer",
		laneId,
		expectedHarness: laneId === "docs" ? "pi" : "opencode",
		runId: randomUUID(),
		status: "failed",
		controllerError: "lane failed",
		...overrides,
	});
	const base = (allocation) => ({
		...
		workers: [worker("docs"), worker("audit")],
		autoMerged: false,
	});
	const first = await store.allocate();
	await store.persist(base(first));
	// A declared expectedHarness that disagrees with its lane is rejected.
	const second = await store.allocate();
	await assert.rejects(store.persist({
		...base(second),
		workers: [worker("docs"), worker("audit", { expectedHarness: "pi" })],
	}), /does not match its effective lane identity/);
	// A completed worker that declares expectedHarness must show verified harness.
	const third = await store.allocate();
	await assert.rejects(store.persist({
		...base(third),
		status: "completed",
		workers: [worker("docs", { status: "completed", harness: "pi" }), worker("audit", { status: "completed" })],
	}), /lacks verified harness identity/);
});
