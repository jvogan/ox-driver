...
test("herd lane-spec runs distinct objectives, routes, agents, checks, timeouts, and ceilings", async () => {
	...
	const result = await invokeLanes({ version: 1, lanes: [
		{ id: "build", role: "builder", objective: "build the parser", workerPath: workers[0], harness: "opencode", route: "route-a", agent: "agent-a", childAgents: ["researcher-a", "reviewer-a"], checks: ["build-check"], timeoutSeconds: 600, costCeilingUsd: 0.02 },
		{ id: "attack", role: "adversary", objective: "break the parser", workerPath: workers[1], harness: "opencode", route: "route-b", agent: "agent-b", checks: ["attack-check"], timeoutSeconds: 300, costCeilingUsd: 0.01 },
		{ id: "docs", role: "documenter", objective: "document the parser", workerPath: workers[2], harness: "opencode", route: "route-b", agent: "agent-doc", checks: ["docs-check"], timeoutSeconds: 900, costCeilingUsd: 0.02 },
	]}, [], runner);
...
test("herd lane-spec rejects an invalid plan before allocation or spawn", async () => {
	const { runner, workers } = await laneFixture();
	const result = await invokeLanes({ version: 1, lanes: [
		{ id: "same", role: "builder", objective: "build", workerPath: workers[0], harness: "opencode", checks: ["check"] },
		{ id: "same", role: "adversary", objective: "break", workerPath: workers[1], harness: "opencode", checks: ["check"] },
	]}, [], runner);
	assert.equal(result.code, 1);
...
test("herd lane-spec appends shared checks and rejects mixed homogeneous selectors", async () => {
	...
	const planDocument = { version: 1, lanes: [
		{ id: "build", role: "builder", objective: "build", workerPath: workers[0], harness: "opencode", checks: ["lane-check"] },
		{ id: "guard", role: "guardian", objective: "guard", workerPath: workers[1], harness: "opencode" },
	]};
...
test("herd runs a mixed plan while keeping shell checks off read-only Pi lanes", async () => {
	const { runner, workers } = await laneFixture();
	const piRoot = await trackedMkdtemp(join(tmpdir(), "ox-herd-pi-runner-"));
	const piRunner = join(piRoot, "pi-runner.mjs");
await writeFile(piRunner, `
import { basename } from "node:path";
const checks = process.argv.flatMap((item, index) => item === "--check" ? [process.argv[index + 1]] : []);
...
process.stdout.write(JSON.stringify({
  ...
  costReport: { observedUsdMicros: 3000 }, changedPaths: [], unownedChangedPaths: [],
  acceptance: checks.map((command) => ({ command, passed: true })), finalOutput: "pi review of " + basename(process.argv[2])
}));
`, { mode: 0o700 });
	await chmod(piRunner, 0o700);
	const planDocument = { version: 1, lanes: [
		{ id: "build", role: "builder", objective: "build the parser", workerPath: workers[0], harness: "opencode", checks: ["build-check"] },
		{ id: "review", role: "reviewer", objective: "review the parser while it is built", workerPath: workers[1], harness: "pi" },
	]};
	if (!OX_DRIVER_SUPPORTS_PI_LANES) {
		const publicPreview = await invokeLanes(planDocument, [], runner, { OX_DRIVER_PI_LANE_RUNNER: piRunner });
		assert.equal(publicPreview.code, 1);
		assert.match(publicPreview.stderr, /does not include Pi lane support/);
		assert.equal(publicPreview.stderr.includes("OX_DRIVER_ORCHESTRATION_ID"), false);
		return;
	}
	const result = await invokeLanes(planDocument, ["--check", "shared-writer-check"], runner, { OX_DRIVER_PI_LANE_RUNNER: piRunner });
	assert.equal(result.code, 0, result.stderr);
	assert.equal(result.receipt.status, "completed");
	assert.deepEqual(result.receipt.workers.map((worker) => worker.harness), ["opencode", "pi"]);
	assert.deepEqual(result.receipt.runners.map((identity) => identity.harness), ["opencode", "pi"]);
	assert.ok(result.receipt.runners.every((identity) => /^[0-9a-f]{64}$/.test(identity.sha256)));
	assert.match(result.receipt.workers[1].finalOutputPreview, /^pi review of /);
	assert.equal(result.receipt.effectivePlan.lanes[0].harness, "opencode");
	assert.equal(result.receipt.effectivePlan.lanes[1].harness, "pi");
	assert.equal(result.receipt.effectivePlan.lanes[1].route, "pi-default");
	assert.deepEqual(result.receipt.workers[1].acceptance, []);
	assert.deepEqual(result.receipt.workers[0].acceptance.map((check) => check.command), ["build-check", "shared-writer-check"]);
	// Runner selection: only the writer lane went through the OpenCode runner,
	// which is the only fake that writes an echo file.
	assert.equal(existsSync(join(result.echoDir, `${basename(workers[0])}.json`)), true);
	assert.equal(existsSync(join(result.echoDir, `${basename(workers[1])}.json`)), false);
	// The stderr contract stays exactly one orchestration id line.
	assert.equal(result.stderr, `OX_DRIVER_ORCHESTRATION_ID=${result.receipt.orchestrationId}\n`);

	// A writer lane without any check is still refused before allocation.
	const uncheckedWriter = await invokeLanes({ version: 1, lanes: [
		{ id: "build", role: "builder", objective: "build", workerPath: workers[0], harness: "opencode" },
		{ id: "review", role: "reviewer", objective: "review", workerPath: workers[1], harness: "pi" },
	]}, [], runner, { OX_DRIVER_PI_LANE_RUNNER: piRunner });
	assert.equal(uncheckedWriter.code, 1);
	assert.match(uncheckedWriter.stderr, /lane build requires at least one plan check/);
	assert.equal(uncheckedWriter.stderr.includes("OX_DRIVER_ORCHESTRATION_ID"), false);


	for (const [mutation, pattern] of [
		[{ agent: "writer" }, /agent is unavailable for Pi lanes/],
	]) {
		const invalid = await invokeLanes({ version: 1, lanes: [
			planDocument.lanes[0],
			{ ...planDocument.lanes[1], ...mutation },
		]}, [], runner, { OX_DRIVER_PI_LANE_RUNNER: piRunner });
		assert.equal(invalid.code, 1);
		assert.match(invalid.stderr, pattern);
		assert.equal(invalid.stderr.includes("OX_DRIVER_ORCHESTRATION_ID"), false);
	}

	const explicitPiCheck = await invokeLanes({ version: 1, lanes: [
		planDocument.lanes[0],
		{ ...planDocument.lanes[1], checks: ["pi-review-check"] },
	]}, [], runner, { OX_DRIVER_PI_LANE_RUNNER: piRunner });
	assert.equal(explicitPiCheck.code, 1);
	assert.match(explicitPiCheck.stderr, /checks is unavailable for read-only Pi lanes/);
	assert.equal(explicitPiCheck.stderr.includes("OX_DRIVER_ORCHESTRATION_ID"), false);
});
