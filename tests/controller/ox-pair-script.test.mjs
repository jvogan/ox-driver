...
const FOREIGN_RECEIPT_RUN_ID = `${"1".repeat(8)}-1111-4111-8111-${"1".repeat(12)}`;
const FOREIGN_MARKER_RUN_ID = `${"2".repeat(8)}-2222-4222-8222-${"2".repeat(12)}`;
...
if (name.includes("slow")) {
  ...
  const receipt = {
    version: 1,
    runId: name.includes("wrong-run-id") ? "${FOREIGN_RECEIPT_RUN_ID}" : process.env.OX_DRIVER_REQUESTED_RUN_ID,
    ...(name.includes("missing-harness") ? {} : { harness: name.includes("wrong-harness") ? "pi" : "opencode" }),
  ...
  if (name.includes("wrong-marker")) process.stderr.write("OX_DRIVER_RUN_ID=${FOREIGN_MARKER_RUN_ID}\\n");
